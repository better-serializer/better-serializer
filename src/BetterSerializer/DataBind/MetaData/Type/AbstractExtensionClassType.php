 ...
 *
 */
abstract class AbstractExtensionClassType extends AbstractClassType implements ExtensionClassTypeInterface
{
    ...
    {
        if (!$type instanceof ExtensionClassTypeInterface || !parent::equals($type)) {
            return false;
        }
    ...
    {
        if ($this->isClass() && $type instanceof ClassType) {
            return $this->isThisClassCompatibleWithClass($type);
        } elseif ($type instanceof InterfaceType) {
            return $this->isCompatibleWithInterface($type);
        } elseif ($this->isInterface() && $type instanceof AbstractClassType) {
    ...
    {
        $this->type = TypeEnum::CUSTOM_CLASS_TYPE();
    }
     ...
     * @return bool
     */
    private function isThisClassCompatibleWithClass(ClassType $type): bool
    {
    ...
    /**
     * @param AbstractClassType $type
     * @return bool
     */
    private function isThisInterfaceCompatibleWithClass(AbstractClassType $type): bool
    {
